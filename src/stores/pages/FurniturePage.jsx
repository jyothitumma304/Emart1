...
const FurniturePage = () => {
  ...
  const companies = [...new Set(furnitureData.map((phone) => phone.company))];
  ...
  const filteredMobiles =
    selectedProduct.length === 0
      ? furnitureData
      : furnitureData.filter((phone) => selectedProduct.includes(phone.company));
    ...
    <>
      ...
      <div className="pageSection">
        {filteredMobiles.map((item) => (
          <Link to={`/Furniture/${item.id}`} key={item.id} className="pageImg">
