...
const FridgePage = () => {
  ...
  const companies = [...new Set(fridgeData.map((phone) => phone.company))];
  ...
  const filteredMobiles =
    selectedProduct.length === 0
      ? fridgeData
      : fridgeData.filter((phone) => selectedProduct.includes(phone.company));
    ...
    <>
      ...
      <div className="pageSection">
        {filteredMobiles.map((item) => (
          <Link to={`/Fridge/${item.id}`} key={item.id} className="pageImg">
