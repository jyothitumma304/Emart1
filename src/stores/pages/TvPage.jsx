...
const TvPage = () => {
  ...
  const companies = [...new Set(tvData.map((phone) => phone.company))];
  ...
  const filteredMobiles =
    selectedProduct.length === 0
      ? tvData
      : tvData.filter((phone) => selectedProduct.includes(phone.company));
    ...
    <>
      ...
      <div className="pageSection">
        {filteredMobiles.map((item) => (
          <Link to={`/TV/${item.id}`} key={item.id} className="pageImg">
