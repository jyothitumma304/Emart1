...
const WomenPage = () => {
  ...
  const companies = [...new Set(womenwearData.map((phone) => phone.company))];
  ...
  const filteredMobiles =
    selectedProduct.length === 0
      ? womenwearData
      : womenwearData.filter((phone) => selectedProduct.includes(phone.company));
    ...
    <>
      ...
      <div className="pageSection">
        {filteredMobiles.map((item) => (
          <Link to={`/Woman Wear/${item.id}`} key={item.id} className="pageImg">
